...
export const events = {
    "Transfer(address,address,uint256,address,bytes,bytes)": event("0x8988d59efc2c4547ef86c88f6543963bab0cea94f8e486e619c7c3a790db93be", "Transfer(address,address,uint256,address,bytes,bytes)", {"from": indexed(p.address), "to": indexed(p.address), "tokenId": indexed(p.uint256), "operator": p.address, "userData": p.bytes, "operatorData": p.bytes}),
    "Transfer(address,address,uint256,address,bytes)": event("0xd5c97f2e041b2046be3b4337472f05720760a198f4d7d84980b7155eec7cca6f", "Transfer(address,address,uint256,address,bytes)", {"from": indexed(p.address), "to": indexed(p.address), "tokenId": indexed(p.uint256), "operator": p.address, "userData": p.bytes}),
    "Transfer(address,address,uint256)": event("0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef", "Transfer(address,address,uint256)", {"from": indexed(p.address), "to": indexed(p.address), "tokenId": indexed(p.uint256)}),
}
